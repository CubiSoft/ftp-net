...
using CubiSoft.Ftp.Messages;

namespace CubiSoft.Ftp
{
    ...
    public class FtpClient : IDisposable
    {
        ...
        /// </summary>
        private StreamChannel m_Stream;
        ...
        /// 
        /// </summary>
        protected StreamChannel BaseStream
        {
            get { return m_Stream; }
        }
        ...
        internal async Task AuthenticateAsync(NetworkCredential credential)
        {
            ...
            if (!(reply = await m_Stream.SetUserAsync(credential.UserName)).Success)
                Debug.WriteLine(reply.Message);

            if (reply.Type == FtpResponseType.PositiveIntermediate &&
                !(reply = await m_Stream.SetPasswordAsync(credential.Password)).Success)
                Debug.WriteLine(reply.Message);
        }
        ...
        public async Task CreateDirectoryAsync(string path, bool force = true)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("CreateDirectory(\"{0}\", true): Create non-existent parent: {1}",
